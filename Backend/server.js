...
app.post("/api/employeeregister", async (req, res) => {
  ...
  try {
    ...
    db.query(
      ...
      async (err, result) => {
        // Use async to allow await within this block
        if (err) {
        ...
        if (result.affectedRows > 0) {
          ...
          const emailSubject =
            "Registration Successful - Leave Management System";
        ...
        if (result.affectedRows > 0) {
          ...
          try {
            ...
            return res.status(201).send({
              message:
                "Manager has been added successfully and confirmation email sent.",
            });
          } catch (emailError) {
            console.error("Error sending email:", emailError);
            return res.status(500).json({
              success: false,
              message:
                "Manager added successfully, but failed to send confirmation email.",
            });
          }
        } else {
          return res.status(404).json({
...
whatsappClient.on("qr", (qr) => {
  // Generate QR code in terminal for debugging purposes
  qrcode.generate(qr, { small: true });
  console.log("QR code generated, storing in the database...");

  // Store the QR code in the MySQL database
  db.query(
    "INSERT INTO whatsapp_connection (status, qr_code) VALUES (?, ?)",
    ["PENDING", qr],
    (err, result) => {
      if (err) throw err;
      console.log("QR code saved in the database.");
    }
  );
});
...
whatsappClient.on("ready", () => {
  ...
  // Update status to 'CONNECTED' and clear the QR code from the database
  db.query(
    "UPDATE whatsapp_connection SET status = ?, qr_code = NULL WHERE status = ?",
    ["CONNECTED", "PENDING"],
    (err, result) => {
      if (err) throw err;
...
// API to retrieve and display the WhatsApp QR code
app.get("/api/whatsapp/generate-qr", (req, res) => {
  db.query(
    "SELECT status, qr_code FROM whatsapp_connection ORDER BY id DESC LIMIT 1",
    (err, result) => {
      if (err) {
        console.error("Error fetching WhatsApp status:", err); // Log error
        return res.status(500).send("Error fetching WhatsApp status.");
      }

      if (result.length > 0 && result[0].qr_code) {
        console.log("QR Code fetched from database:", result[0].qr_code); // Log fetched QR code
        res.json({ qr_code: result[0].qr_code });
      } else {
        console.log("QR code not available or already connected."); // Log if no QR code found
        res
          .status(404)
          .json({ message: "QR code not available or already connected." });
      }
    }
  );
});
...
app.post("/api/whatsapp/send-message", async (req, res) => {
  ...
  try {
    // Format the phone number for WhatsApp (country code + number)
    const formattedNumber = `${number}@c.us`;

    // Send the message via the WhatsApp client
...
app.put("/api/LeaveApply/:id/:action", (req, res) => {
  ...
  db.query(
    ...
    (error, results) => {
      ...
      // Fetch the user's email and handphone number in parallel
      db.query(
        `SELECT u.username AS email, u.handphone 
         FROM users u 
         JOIN leave_applications l ON u.username = l.username 
         WHERE l.id = ?`,
        [id],
        async (error, results) => {
          if (error) {
            console.error("Error fetching email and handphone:", error);
          ...
          const { email: employeeEmail, handphone } = results[0];
            ...
            <html>
              <body>
                ...
                <p style="color: black;">
                  ...
                  <a href="https://lms-model.netlify.app/myleave">My Leave Status</a>.
                </p>
                <p style="color: black;">
          ...
          try {
            ...
            const formattedNumber = `${countryCode}${handphone.slice(1)}@c.us`;
            const message = `Dear Employee, your leave request has been ${action}. Check details: https://lms-model.netlify.app/myleave`;
            ...
            res.json({
              message: "Leave request updated successfully, email and WhatsApp message sent",
            });
          } catch (notificationError) {
            console.error("Error during notification:", notificationError);
            res.status(500).json({
              error: "Leave request updated, but failed to send notifications",
            });
          }
        }
      );
...
app.get("/api/whatsapp/test-send", async (req, res) => {
  const testNumber = "94760266276"; // Replace with a valid test number
  const testMessage = "This is a test message from the server.";

  try {
    await whatsappClient.sendMessage(testNumber + "@c.us", testMessage);
    return res.status(200).send("Test message sent successfully.");
  } catch (error) {
    console.error("Error sending test message:", error.message);
    return res
      .status(500)
      .send("Failed to send test message: " + error.message);
  }
});
